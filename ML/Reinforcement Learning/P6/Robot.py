...
class Robot(object):
    ...
    def __init__(self, maze, alpha=0.5, gamma=0.9, epsilon0=0.5, explicit_exploration_count = 0):
        ...
        # the first several times of exploration could all be randomly picked action
        self.explicit_exploration_count = explicit_exploration_count
    ...
    def update_parameter(self):
        ...
        if self.testing:
            ...
            self.t += 1
            if (self.t <= self.explicit_exploration_count):
                self.epsilon = 1
            else:
                # self.epsilon = self.epsilon0 / ( 1 + self.t) if self.epsilon_linear_decay else self.epsilon0 / ( 1 + math.sqrt(self.t))
                self.epsilon = self.epsilon0 / ( 1 + self.t - self.explicit_exploration_count) if self.epsilon_linear_decay else self.epsilon0 / ( 1 + math.sqrt(self.t - self.explicit_exploration_count))
